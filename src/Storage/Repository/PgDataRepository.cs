    ...
    : IDataRepository
{
    ...
    )
    {
        ...
        await using NpgsqlCommand pgcom = BuildUpdateCommand(
            new DataElementUpdateCommandArguments(
                dataElementId,
                instanceGuid,
    ...
    )
    {
        ...
        await using NpgsqlCommand pgcom = BuildUpdateCommand(
            new DataElementUpdateCommandArguments(
                ...
                CustomSerializer.Serialize(new InstanceInternal(), new List<string>()),
                false,
                DBNull.Value,
                DBNull.Value,
    ...
    private sealed record DataElementUpdateCommandArguments(
        Guid DataElementId,
        Guid InstanceGuid,
        string ElementChanges,
        string InstanceChanges,
        bool IsReadChangedToFalse,
        object LastChanged,
        object NewCurrentBlobVersion,
        object ExpectedCurrentBlobVersion,
        bool EnforceLockCheck
    );

    private NpgsqlCommand BuildUpdateCommand(DataElementUpdateCommandArguments arguments)
    {
        NpgsqlCommand command = _dataSource.CreateCommand(_updateSql);
        command.Parameters.AddWithValue(NpgsqlDbType.Uuid, arguments.DataElementId);
        command.Parameters.AddWithValue(NpgsqlDbType.Uuid, arguments.InstanceGuid);
        command.Parameters.AddWithValue(NpgsqlDbType.Jsonb, arguments.ElementChanges);
        command.Parameters.AddWithValue(NpgsqlDbType.Jsonb, arguments.InstanceChanges);
        command.Parameters.AddWithValue(NpgsqlDbType.Boolean, arguments.IsReadChangedToFalse);
        command.Parameters.AddWithValue(NpgsqlDbType.TimestampTz, arguments.LastChanged);
        command.Parameters.AddWithValue(NpgsqlDbType.Uuid, arguments.NewCurrentBlobVersion);
        command.Parameters.AddWithValue(NpgsqlDbType.Uuid, arguments.ExpectedCurrentBlobVersion);
        command.Parameters.AddWithValue(NpgsqlDbType.Boolean, arguments.EnforceLockCheck);
        return command;
    }
