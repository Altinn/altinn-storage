    ...
    : IClassFixture<TestApplicationFactory<InstancesController>>
{
    ...
    public async Task Get_One(bool isPortalUser, HttpStatusCode expectedStatusCode)
    {
        // Arrange
        int instanceOwnerPartyId = 1337;
        ...
        HttpClient client = GetTestClient();
        string token = PrincipalUtil.GetToken(3, 1337, 3, isPortalUser: isPortalUser);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ...
    public async Task Get_One_Twice_Ok()
    {
        ...
        HttpClient client = GetTestClient();
        string token = PrincipalUtil.GetToken(3, 1337, 3, isPortalUser: true);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ...
    public async Task Delete_EndUserSoftDeletesInstance_ReturnedInstanceHasStatusOnlySoftDeleted()
    {
        ...
        HttpClient client = GetTestClient();
        string token = PrincipalUtil.GetToken(1337, 1337, isPortalUser: true);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ...
    public async Task UpdateReadStatus_SetInitialReadStatus_ReturnsUpdatedInstance()
    {
        ...
        string token = PrincipalUtil.GetToken(3, 1337, 2, isPortalUser: true);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
    ...
    public async Task UpdatePresentationFields_NoPreviousFieldsSet_ReturnsUpdatedInstance()
    {
        ...
        string token = PrincipalUtil.GetToken(3, 1337, isPortalUser: true);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        HttpRequestMessage httpRequestMessage = new HttpRequestMessage(HttpMethod.Put, requestUri)
        {
