...
{
    private static readonly MethodInfo CreateApplyMutationExceptionMethod =
        typeof(PgInstanceMutationRepository).GetMethod(
            "CreateApplyMutationException",
            BindingFlags.NonPublic | BindingFlags.Static
        );

    private static readonly MethodInfo EnsureReplaySnapshotMatchesAdmissionMethod =
        typeof(PgInstanceMutationRepository).GetMethod(
            "EnsureReplaySnapshotMatchesAdmission",
            BindingFlags.NonPublic | BindingFlags.Static
        );

    private static readonly MethodInfo BuildCreateElementsPayloadMethod = GetStaticPayloadBuilder(
        "BuildCreateElementsPayload"
    );

    private static readonly MethodInfo BuildUpdateElementsPayloadMethod = GetStaticPayloadBuilder(
        "BuildUpdateElementsPayload"
    );

    private static readonly MethodInfo BuildDeleteElementsPayloadMethod = GetStaticPayloadBuilder(
        "BuildDeleteElementsPayload"
    );

    private static readonly MethodInfo BuildInstanceUpdatesPayloadMethod = GetStaticPayloadBuilder(
        "BuildInstanceUpdatesPayload"
    );

    private static readonly MethodInfo BuildEventsPayloadMethod = GetStaticPayloadBuilder(
        "BuildEventsPayload"
    );

    private static readonly MethodInfo BuildOutboxPayloadMethod =
        typeof(PgInstanceMutationRepository).GetMethod(
            "BuildOutboxPayload",
    ...
    public void BuildPayloads_RepresentativeMutation_WritesSemanticJsonStructure()
    {
        ...
        string createPayload = InvokeStaticPayloadBuilder(
            BuildCreateElementsPayloadMethod,
            mutation.CreateDataElements
        );
        string updatePayload = InvokeStaticPayloadBuilder(
            BuildUpdateElementsPayloadMethod,
            mutation.UpdateDataElements
        );
        string deletePayload = InvokeStaticPayloadBuilder(
            BuildDeleteElementsPayloadMethod,
            mutation.DeleteDataElements
        );
        string instanceUpdatesPayload = InvokeStaticPayloadBuilder(
            BuildInstanceUpdatesPayloadMethod,
            mutation
        );
        string eventsPayload = InvokeStaticPayloadBuilder(
            BuildEventsPayloadMethod,
            instanceGuid,
            mutation
        );
    ...
    public void BuildInstanceUpdatesPayload_ProcessEndArchive_WritesProcessAndStatusInFlatObject()
    {
        ...
        string payload = InvokeStaticPayloadBuilder(BuildInstanceUpdatesPayloadMethod, mutation);
    ...
    public void BuildInstanceUpdatesPayload_MultipleBranches_WritesOneFlatObject()
    {
        ...
        AssertSharedInstanceUpdateScalars(root, lastChanged, "Task_Shape", true);
    ...
    public void CreateApplyMutationException_IdempotencyKeyInstanceMismatch_ReturnsConflict()
    {
        ...
        RepositoryException exception = Assert.IsType<RepositoryException>(
            CreateApplyMutationExceptionMethod.Invoke(null, [instanceGuid, postgresException])
        );
    ...
    public void ReplaySnapshotVersionDrift_ReturnsVersionMismatchWithActualSnapshotVersions()
    {
        ...
        TargetInvocationException reflectionException = Assert.Throws<TargetInvocationException>(
            () =>
                EnsureReplaySnapshotMatchesAdmissionMethod.Invoke(null, [instance, 13, 9])
        );

        InstanceVersionMismatchException exception =
            Assert.IsType<InstanceVersionMismatchException>(reflectionException.InnerException);
    ...
    private static UnreachableException AssertApplyMutationContractDrift(string messageText)
    {
        ...
        TargetInvocationException reflectionException = Assert.Throws<TargetInvocationException>(
            () =>
                CreateApplyMutationExceptionMethod.Invoke(null, [Guid.NewGuid(), postgresException])
        );

        UnreachableException exception = Assert.IsType<UnreachableException>(
    ...
    private static MethodInfo GetStaticPayloadBuilder(string methodName) =>
        typeof(PgInstanceMutationRepository).GetMethod(
            methodName,
            BindingFlags.NonPublic | BindingFlags.Static
        );

    private static string InvokeStaticPayloadBuilder(MethodInfo method, params object[] args) =>
        Assert.IsType<string>(method.Invoke(null, args));
    ...
    private static string InvokeOutboxPayload(Guid instanceGuid, InstanceMutationCommit mutation)
    {
        ...
        PgInstanceMutationRepository repository = new(null, null, outboxInsertRowFactory);

        return Assert.IsType<string>(
            BuildOutboxPayloadMethod.Invoke(repository, [instanceGuid, mutation])
        );
    }
