CREATE OR REPLACE FUNCTION storage.readinstancesforparty(
...
AS $BODY$
DECLARE
    _from TIMESTAMPTZ := COALESCE(_date_from, '-infinity');
    _to TIMESTAMPTZ := COALESCE(_date_to, 'infinity');
BEGIN
    RETURN QUERY
    ...
    SELECT
        ...
        filteredInstances.created,
        filteredInstances.instance_version,
        filteredInstances.process_state_version,
        d.element,
        d.currentblobversion 
    FROM filteredInstances
    LEFT JOIN storage.dataelements d 
        ON filteredInstances.id = d.instanceInternalId 
