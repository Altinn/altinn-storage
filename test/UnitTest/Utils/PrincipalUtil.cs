...
namespace Altinn.Platform.Storage.UnitTest.Utils
{
    public static class PrincipalUtil
    {
        ...
        public static string GetToken(int userId, int partyId, int authenticationLevel = 2, bool isPortalUser = false, IEnumerable<string> scopes = null)
        {
            var principal = GetPrincipal(userId, partyId, authenticationLevel, isPortalUser, scopes);
            string token = JwtTokenMock.GenerateToken(principal, new TimeSpan(1, 1, 1));

            return token;
        }

        public static ClaimsPrincipal GetPrincipal(int userId, int partyId, int authenticationLevel = 2, bool isPortalUser = false, IEnumerable<string> scopes = null)
        {
            ...
            if (isPortalUser)
            {
                claimScopes.Add(ApiScopeAuthorizer.PortalUserApiScope);
            }
