...
public class DataRepositoryMock : IDataRepository
{
    ...
    private void EnsureDataElementLoaded(Guid dataElementId)
    {
        ...
        (DataElementInternal dataElement, string storedBlobVersion) = ReadDataElementFile(
            dataElementId
        );
    ...
    private static (DataElementInternal DataElement, string CurrentBlobVersion) ReadDataElementFile(
        Guid dataElementId
    )
    {
        lock (TestDataUtil.DataLock)
        {
            ...
            if (File.Exists(elementPath))
            {
                ...
                // The blob version lives in its own column, not in the element JSONB, so the
                // element type does not serialize it; test data carries it as an extra property.
                using JsonDocument document = JsonDocument.Parse(content);
                string currentBlobVersion = document.RootElement.TryGetProperty(
                    "currentBlobVersion",
                    out JsonElement currentBlobVersionElement
                )
                    ? currentBlobVersionElement.GetString()
                    : null;

                return (dataElement, currentBlobVersion);
            }
        }
