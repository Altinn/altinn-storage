...
public class DataTests : IClassFixture<DataElementFixture>
{
    ...
    public async Task DeleteBlobVersions_DeletesExactUnattachedVersionsForDataElement()
    {
        ...
        int deleted = await _dataElementFixture.DataRepo.DeleteBlobVersions(
            dataElementId,
    ...
    public async Task DeleteBlobVersion_AttachedBlobVersion_ReturnsFalseAndKeepsRow()
    {
        ...
        bool deleted = await _dataElementFixture.DataRepo.DeleteBlobVersion(
            Guid.Parse(dataElement.Id),
            blobVersionId
        );
    ...
    public async Task AggregateMutation_MixedOperations_CommitsAllChanges()
    {
        ...
        InstanceInternal updatedInternal = await _dataElementFixture.InstanceRepo.GetOne(
            instanceGuid,
            true,
            CancellationToken.None
        );
        DataElementInternal updatedExisting = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(existing.Id)
        );
    ...
    public async Task AggregateMutation_ApplyWithInstanceEvent_WritesOutboxRow()
    {
        ...
        PgInstanceMutationRepository repository = new(
            null,
            _dataElementFixture.DataSource,
    ...
    public async Task AggregateMutation_DeleteInstanceRetryOnHardDeletedInstance_ReplaysAndDoesNotDuplicateDeletedEvent()
    {
        ...
        RepositoryException replayException = await Assert.ThrowsAsync<RepositoryException>(() =>
            _dataElementFixture.InstanceMutationRepo.TryReplayAdmission(
                instanceGuid,
                previousInstanceVersion,
    ...
    public async Task AggregateMutation_StaleContentVersion_RollsBackEarlierOperations()
    {
        ...
        // Act
        await Assert.ThrowsAsync<DataElementBlobVersionMismatchException>(() =>
            _dataElementFixture.InstanceMutationRepo.Apply(
                instanceGuid,
                _instanceInternalId,
                mutation
            )
        ...
        // Assert
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(toCreate.Id)));
        DataElementInternal unchangedExisting = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(existing.Id)
        );
    ...
    public async Task AggregateMutation_UpdateHardDeletedDataElement_MapsToDataElementNotUpdated()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement hardDeletedElement = TestDataUtil.GetDataElement(DataElement1);
    ...
    public async Task AggregateMutation_IdempotentReplay_ReturnsFirstCommittedBlob()
    {
        ...
        InstanceMutationApplyResult earlyReplayResult =
            await _dataElementFixture.InstanceMutationRepo.TryReplayAdmission(
                instanceGuid,
                previousInstanceVersion,
                firstResult.Instance.Versions.InstanceVersion,
                firstResult.Instance.Versions.ProcessStateVersion,
    ...
    public async Task AggregateMutation_ProcessEndArchive_CommitsProcessAndStatusInOneInstanceUpdate()
    {
        ...
        // Act
        InstanceMutationApplyResult result = await _dataElementFixture.InstanceMutationRepo.Apply(
            instanceGuid,
            _instanceInternalId,
            mutation,
            CancellationToken.None
        );
        InstanceInternal updatedInstanceInternal = await _dataElementFixture.InstanceRepo.GetOne(
            instanceGuid,
            false,
            CancellationToken.None
        );
    ...
    public async Task ApplyInstanceMutationSql_MixedMutationReplayAndOutbox_CommitsAndReplays()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement toUpdate = TestDataUtil.GetDataElement(DataElement1);
        ...
        InstanceInternal updatedInstance = await _dataElementFixture.InstanceRepo.GetOne(
            instanceGuid,
            false,
            CancellationToken.None
        );
        ...
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(toDelete.Id)));
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(firstCreate.Id)));
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(secondCreate.Id)));
        Assert.Equal(
            "updated",
        ...
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(retryCreate.Id)));
        Assert.Equal(
            1,
            await CountInstanceEvents(instanceGuid, InstanceEventType.Saved.ToString())
        );
    ...
    public async Task ApplyInstanceMutationSql_MultipleCreates_PreservesOrderAndBumpsOnce()
    {
        ...
        DataElementInternal createdFirstElement = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(firstCreate.Id)
        );
        DataElementInternal createdSecondElement = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
    ...
    public async Task ApplyInstanceMutationSql_CreateWithoutLastChanged_StampsMutationTimestampAndStripsNullActor()
    {
        ...
        DataElementInternal createdElement = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
        ...
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(toCreate.Id)));
    ...
    public async Task ApplyInstanceMutationSql_MultipleDeletes_PreservesOrderAndBumpsOnce()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement firstDelete = TestDataUtil.GetDataElement(DataElement1);
        ...
        DataElement secondDelete = TestDataUtil.GetDataElement(DataElement2);
        ...
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(firstDelete.Id)));
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(secondDelete.Id)));
    ...
    public async Task ApplyInstanceMutationSql_DeleteWithNullLastChangedBy_DoesNotCollapseDocument()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement toDelete = TestDataUtil.GetDataElement(DataElement1);
    ...
    public async Task ApplyInstanceMutationSql_MultipleUpdates_PreservesOrderAndPerUpdateInstanceEffects()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement firstUpdate = TestDataUtil.GetDataElement(DataElement1);
        (firstUpdate, _) = await CreateVersionedDataElement(firstUpdate);
        DataElement secondUpdate = TestDataUtil.GetDataElement(DataElement2);
        (secondUpdate, _) = await CreateVersionedDataElement(secondUpdate);
        DataElement thirdUpdate = TestDataUtil.GetDataElement(DataElement3);
        ...
        DataElementInternal updatedSecondElement = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(secondUpdate.Id)
        );
        DataElementInternal updatedFirstElement = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(firstUpdate.Id)
        );
        DataElementInternal updatedThirdElement = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(thirdUpdate.Id)
        );
    ...
    public async Task ApplyInstanceMutationSql_UpdateReadStatus_UsesFinalStateWhenTrueThenFalse()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement elementA = TestDataUtil.GetDataElement(DataElement1);
        elementA.IsRead = true;
        elementA = await CreateLegacyDataElement(elementA);
        DataElement elementB = TestDataUtil.GetDataElement(DataElement2);
        ...
        DataElementInternal updatedElementA = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(elementA.Id)
        );
        DataElementInternal updatedElementB = await _dataElementFixture.DataRepo.Read(
            instanceGuid,
            Guid.Parse(elementB.Id)
        );
    ...
    public async Task ApplyInstanceMutationSql_UpdateBlobAttachMissingRows_DoesNotValidateAttachCount()
    {
        ...
        Assert.Equal(
            ...
            (
                await _dataElementFixture.DataRepo.Read(instanceGuid, Guid.Parse(firstUpdate.Id))
            ).BlobVersionId
        );
        Assert.Equal(
            secondNewBlobVersion,
            (
                await _dataElementFixture.DataRepo.Read(instanceGuid, Guid.Parse(secondUpdate.Id))
            ).BlobVersionId
        );
    }
    ...
    public async Task ApplyInstanceMutationSql_UpdateBlobVersionMismatch_RollsBackEarlierOperations()
    {
        ...
        Assert.Equal(
            currentVersion,
            (
                await _dataElementFixture.DataRepo.Read(instanceGuid, Guid.Parse(existing.Id))
            ).BlobVersionId
        );
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(toCreate.Id)));
        Assert.Equal(0, await CountAttachedBlobVersionRows(toCreate.BlobVersionId));
    ...
    public async Task ApplyInstanceMutationSql_UpdateValidationRollsBackEarlierCreateWithMissingBlobAttach()
    {
        ...
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElementInternal toCreate = await PrepareAggregateCreateDataElement(instanceGuid);
        DataElement lockedUpdate = TestDataUtil.GetDataElement(DataElement1);
        ...
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(toCreate.Id)));
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(lockedUpdate.Id)));
        Assert.Equal(currentInstanceVersion, await ReadInstanceVersion(instanceGuid));
    }
    ...
    public async Task ApplyInstanceMutationSql_DeleteDataElementNotFound_ReportsFirstDeleteOrdinal()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement missingDelete = TestDataUtil.GetDataElement(DataElement1);
        missingDelete.Id = Guid.NewGuid().ToString();
        missingDelete.InstanceGuid = instanceGuid.ToString();
        DataElement existingDelete = TestDataUtil.GetDataElement(DataElement2);
        ...
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(existingDelete.Id)));
    ...
    public async Task ApplyInstanceMutationSql_DeleteAllowedThenLocked_ReportsLockedTarget()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement allowedDelete = TestDataUtil.GetDataElement(DataElement1);
        (allowedDelete, string allowedBlobVersionId) = await CreateVersionedDataElement(
            allowedDelete
        );
        DataElement lockedDelete = TestDataUtil.GetDataElement(DataElement2);
        ...
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(allowedDelete.Id)));
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(lockedDelete.Id)));
    ...
    public async Task ApplyInstanceMutationSql_DeleteAllowedThenMissing_ReportsMissingTarget()
    {
        ...
        DataElement missingDelete = TestDataUtil.GetDataElement(DataElement2);
        missingDelete.Id = Guid.NewGuid().ToString();
        missingDelete.InstanceGuid = instanceGuid.ToString();
        int currentInstanceVersion = await ReadInstanceVersion(instanceGuid);
        int currentProcessStateVersion = await ReadProcessStateVersion(instanceGuid);
    ...
    public async Task ApplyInstanceMutationSql_DeleteLockedElement_RaisesAm001LockedAndDoesNotDelete()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement lockedDelete = TestDataUtil.GetDataElement(DataElement1);
        lockedDelete.Locked = true;
    ...
    [Fact]
    public async Task ApplyInstanceMutationSql_DeleteLockedElementWithIgnoreLock_Deletes()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement lockedDelete = TestDataUtil.GetDataElement(DataElement1);
        ...
        List<ApplyMutationSqlRow> rows = await ApplyInstanceMutationSql(
            ...
            null,
            null,
            DeleteElementsPayload([lockedDelete], ignoreLock: true),
            null,
            null,
            null
        );
        ...
        // Assert
        AssertAppliedRows(rows, currentInstanceVersion + 1, currentProcessStateVersion);
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(lockedDelete.Id)));
        Assert.Equal(1, await CountDetachedBlobVersionRows(blobVersionId));
    }
    ...
    public async Task ApplyInstanceMutationSql_DeleteHardDeletedDataElement_DoesNotTreatHardDeleteFlagAsLock()
    {
        ...
        // Assert
        AssertAppliedRows(rows, currentInstanceVersion + 1, currentProcessStateVersion);
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(hardDeletedElement.Id)));
        Assert.Equal(1, await CountDetachedBlobVersionRows(blobVersionId));
    }

    [Fact]
    public async Task ApplyInstanceMutationSql_DeleteUnlockedElementWithIgnoreLock_Deletes()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement unlockedDelete = TestDataUtil.GetDataElement(DataElement1);
        (unlockedDelete, string blobVersionId) = await CreateVersionedDataElement(unlockedDelete);
        int currentInstanceVersion = await ReadInstanceVersion(instanceGuid);
        int currentProcessStateVersion = await ReadProcessStateVersion(instanceGuid);
        ...
        List<ApplyMutationSqlRow> rows = await ApplyInstanceMutationSql(
            ...
            null,
            null,
            DeleteElementsPayload([unlockedDelete], ignoreLock: true),
            null,
            null,
            null
        );
        ...
        // Assert
        AssertAppliedRows(rows, currentInstanceVersion + 1, currentProcessStateVersion);
        Assert.False(await _dataElementFixture.DataRepo.Exists(Guid.Parse(unlockedDelete.Id)));
        Assert.Equal(1, await CountDetachedBlobVersionRows(blobVersionId));
    }
    ...
    public async Task ApplyInstanceMutationSql_LockedElement_RaisesAm001LockedAndDoesNotUpdate()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement lockedElement = TestDataUtil.GetDataElement(DataElement1);
    ...
    [Fact]
    public async Task ApplyInstanceMutationSql_CreateOnHardDeletedInstance_RaisesInstanceHardDeletedAndDoesNotCreateElement()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElementInternal toCreate = await PrepareAggregateCreateDataElement(instanceGuid);
    ...
    [Fact]
    public async Task ApplyInstanceMutationSql_UpdateOnHardDeletedInstance_RaisesInstanceHardDeletedAndDoesNotUpdateElement()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement toUpdate = TestDataUtil.GetDataElement(DataElement1);
    ...
    [Fact]
    public async Task ApplyInstanceMutationSql_DeleteOnHardDeletedInstance_RaisesInstanceHardDeletedAndDoesNotDeleteElement()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        DataElement toDelete = TestDataUtil.GetDataElement(DataElement1);
        ...
        AssertSqlErrorHasNoMutationTarget(exception);
        Assert.True(await _dataElementFixture.DataRepo.Exists(Guid.Parse(toDelete.Id)));
        Assert.Equal(1, await CountAttachedBlobVersionRows(blobVersionId));
        Assert.Equal(currentInstanceVersion, await ReadInstanceVersion(instanceGuid));
    }

    [Fact]
    public async Task ApplyInstanceMutationSql_InstanceUpdateOnHardDeletedInstance_RaisesInstanceHardDeletedAndDoesNotUpdateInstance()
    {
        // Arrange
        Guid instanceGuid = Guid.Parse(_instance.Id.Split('/').Last());
        int currentInstanceVersion = await ReadInstanceVersion(instanceGuid);
        ...
        AssertSqlErrorHasNoMutationTarget(exception);
        Assert.False(await InstanceDataValuesContainsKey(instanceGuid, "hardDeletedUpdate"));
        Assert.Equal(currentInstanceVersion, await ReadInstanceVersion(instanceGuid));
    }
    ...
    {
        await using NpgsqlCommand cmd = _dataElementFixture.DataSource.CreateCommand(
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("tryreplayinstancemutation_v2 returned no row.");
        }
    ...
    private async Task<ParityInstance> CreateParityInstance()
    {
        ...
        InstanceInternal created = await _dataElementFixture.InstanceRepo.Create(
    ...
    {
        DataElement dataElement = TestDataUtil.GetDataElement(DataElement3);
