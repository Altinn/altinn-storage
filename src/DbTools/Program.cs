...
internal static class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        File.WriteAllText(
            scriptFile,
            $"-- This script is autogenerated from the tool {toolName.Split(',')[0]}. Do not edit manually.{cr}{cr}"
        );
        foreach (
            string filename in (
                new DirectoryInfo(funcAndProcDirectory)
                    .GetFiles(("*.sql"))
                    .Select(f => f.FullName)
                    .OrderBy(f => f)
            )
        )
        {
            File.AppendAllText(
                scriptFile,
                $"-- {filename.Split(Path.DirectorySeparatorChar)[^1]}:{cr}{File.ReadAllText(filename)}{cr}{cr}"
            );
        }

        Console.WriteLine(
            $"DbTools:{cr}Script: {scriptFile}{cr}Migration dir: {migrationPath}, {Path.GetFullPath(migrationPath)}{cr}"
    ...
    {
        return new DirectoryInfo(migrationPath)
            .GetDirectories("v*")
            .Select(d => d.FullName)
    ...
    private static string GetScriptFile(string versionDirectory)
    {
        ...
        if (scriptFiles.Length > 1)
        {
            throw new ArgumentException(
                $"Multiple ({scriptFiles.Length}) potential script files found in {versionDirectory}"
            );
        }
