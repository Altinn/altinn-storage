...
public class InstanceEventTests : IClassFixture<InstanceEventFixture>
{
    ...
    private readonly InstanceEvent _ie1 = new()
    {
        Id = Guid.NewGuid(),
    ...
    private readonly InstanceEvent _ie2 = new()
    {
        Id = Guid.NewGuid(),
        InstanceId = _instanceId,
        EventType = "et1",
        Created = DateTime.Parse("2004-06-16T11:06:59.0851832Z"),
    };
    private readonly InstanceEvent _ie3 = new()
    {
        Id = Guid.NewGuid(),
        InstanceId = _instanceId,
        EventType = "et2",
        Created = DateTime.Parse("2014-06-16T11:06:59.0851832Z"),
    };
    ...
    public async Task DataElement_DeleteAllInstanceEvents_Ok()
    {
        ...
        string sql =
            $"select count(*) from storage.instanceevents where alternateid = '{_ie1.Id}' or alternateid = '{_ie2.Id}' or alternateid = '{_ie3.Id}';";
...
public class InstanceEventFixture
{
    ...
    public InstanceEventFixture()
    {
        var serviceList = ServiceUtil.GetServices(
            new List<Type>() { typeof(IInstanceEventRepository) }
        );
        InstanceEventRepo = (IInstanceEventRepository)
            serviceList.First(i => i.GetType() == typeof(PgInstanceEventRepository));
    }
}
