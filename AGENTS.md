...
- On CI (`CI=true`) **warnings are treated as errors** and StyleCop (`SA*`) rules are at `error` severity in `.editorconfig`. Code that builds locally with warnings can still fail CI — keep it warning-clean.
