...
) : ControllerBase
{
    ...
    {
        int deletedCount = (
            await DeleteVersionedBlobsAndGetDeletedVersionIdsInternal(
                blobVersion,
                cancellationToken
            )
        ).Count;

        return deletedCount == blobVersion.BlobVersionIds.Count;
    }

    private async Task<IReadOnlyList<string>> DeleteVersionedBlobsAndGetDeletedVersionIdsInternal(
        BlobVersionReferencesInternal blobVersion,
        CancellationToken cancellationToken
    )
    {
        ...
        bool[] results = await blobRepository.DeleteBlobsIfExists(
            blobVersion.BlobStorageOrg,
        ...
        for (int i = 0; i < blobVersion.BlobVersionIds.Count; i++)
        {
            if (i < results.Length && results[i])
            {
                deletedVersionIds.Add(blobVersion.BlobVersionIds[i]);
            }
        }
    ...
    )
    {
        ...
        )
        {
            ...
            IReadOnlyList<string> deletedVersionIds =
                await DeleteVersionedBlobsAndGetDeletedVersionIdsInternal(
                    orphanBlobVersion,
                    cancellationToken
                );
